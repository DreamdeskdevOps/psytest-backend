...
const createTest = async (testData, adminId, ipAddress, userAgent) => {
  try {
    ...
    // Log admin activity
    await TestModel.logAdminActivity(adminId, 'TEST_CREATE', {
      testId: newTest.id,
    ...
    const responseData = {
      ...newTest,
      settings: newTest.settings ? JSON.parse(newTest.settings) : {}
    };
...
const duplicateTest = async (originalId, newTitle, adminId, ipAddress, userAgent) => {
  try {
    ...
    const existingWithTitle = await TestModel.findByTitle(newTitle);
    ...
    return generateResponse(
      ...
      {
        ...duplicatedTest,
        settings: duplicatedTest.settings ? JSON.parse(duplicatedTest.settings) : {}
      }, 
      201
    );
  ...
  try {
    ...
    for (const testId of testIds) {
      try {
        ...
        switch (operation) {
          case 'activate':
            result = await TestModel.adminToggleStatus(testId, adminId);
            if (!result.is_active) {
              result = await TestModel.adminToggleStatus(testId, adminId); // Toggle again to activate
            }
            break;
          ...
          case 'deactivate':
            result = await TestModel.adminToggleStatus(testId, adminId);
            if (result.is_active) {
              result = await TestModel.adminToggleStatus(testId, adminId); // Toggle again to deactivate
            }
            break;
            
          case 'delete':
            result = await TestModel.adminDeleteTest(testId, adminId);
            break;
            
          case 'update_category':
            result = await TestModel.updateTestConfig(testId, { category: operationData.category }, adminId);
            break;
    ...
    // Log bulk operation
    await TestModel.logAdminActivity(adminId, 'BULK_OPERATION', {
      operation: operation,
      testIds: testIds,
